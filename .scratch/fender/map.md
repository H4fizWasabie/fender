...
- [17-AskTool](issues/17-AskTool.md) — D49: ask tool — one agent, subagent = one API call to another key; parallel asks; live cross-key verified.
- [16-ParallelSubagents](issues/16-ParallelSubagents.md) — D48: parallel tool dispatch, subagent identity + live streams ([subagent:zen-2] tags), `subagent =` config provider; live-verified cross-account keys. 
