...
1. **Read `DECISIONS.md`** — the decision log (D1–D50). Every design rule below is a decision. If a decision is missing, ADD it to DECISIONS.md in the same commit — never build on an unrecorded decision.
...
- **One provider surface.** OpenAI-compatible endpoints and the shipped Anthropic adapter both translate through the same internal request/response shape. A configured backup key is provider fallback, never another agent. (D6, D42, D50)
- **One main agent, one Agent type.** `delegate` synchronously creates one ephemeral child with isolated working state; the child cannot delegate. Tools run sequentially. No planner, reflection phase, swarm, or parallel-execution machinery. (D13, D50)
...
- **Child isolation.** A child reads canonical project memory but owns fresh conversation, artifact context, and working state; it never persists or consolidates a child session. (D50)
...
- **Deferred list in the spec is a NO-BUILD list.** TUI and response caching remain deferred. Seams only unless a later decision explicitly supersedes them.
...
fender/
  ...
  internal/
    agent/              # ONE main loop + ephemeral child-as-a-tool
    provider/           # provider clients + fallback chain + TOML registry
